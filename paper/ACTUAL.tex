...
\]
where $\varepsilon_{\mathrm{global}}^K=\min_{P,\mu}\sum_i\|(I{-}P)(s^{(i)}{-}\mu)\|_w^2$
and $\varepsilon_{\mathrm{TEMPO}}^K=\sum_m\min_{P_m,\mu_m}\sum_i\gamma_{im}\|(I{-}P_m)(s^{(i)}{-}\mu_m)\|_w^2$
...
BIC on the TEMPO EM log-likelihood (lower is better, penalizes model complexity)
...
TEMPO(POD) trained jointly outperforms the joint POD-DeepONet baseline by 31--50\% on Burgers' and up to $3\times$ on Darcy flow, while discovered regimes span multiple parameter values, confirming that the partition reflects solution geometry and physical structure jointly.
...
Extending TEMPO to higher-dimensional benchmarks will test whether regime discovery generalizes beyond one-dimensional shock dynamics.
Convergence guarantees for Generalized EM under nonlinear basis parameterization remain an open theoretical problem.
...
\caption{POD-DeepONet: representative reconstructions on 1D Burgers'
  ...
  (\textit{center}), point-wise absolute error (\textit{right}).}
...
\caption{TEMPO(POD) on 1D Burgers' ($M{=}3$): ground truth (\textit{left}),
  prediction (\textit{center}), point-wise absolute error (\textit{right}).}
...
\end{align*}
Since $\sum_i\gamma_{im}(s^{(i)}{-}\bar{s})=N_mc_m$,
the cross-term (second line) equals $-2N_m\|(I{-}P)c_m\|_w^2$.
Adding the quadratic term (third line) gives $-N_m\|(I{-}P)c_m\|_w^2$,
which is the right-hand side of the identity.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:advantage}]
Let $P^*,\bar{s}$ be the globally optimal rank-$K$ projector and mean,
and $P_m^*,\bar{s}_m$ their regime-$m$ counterparts.
...
\]
By Lemma~\ref{lem:mean}, the inner sum for each $m$ satisfies
\[
  \sum_i\gamma_{im}\|(I{-}P^*)(s^{(i)}{-}\bar{s}_m)\|_w^2
  = \sum_i\gamma_{im}\|(I{-}P^*)(s^{(i)}{-}\bar{s})\|_w^2
  - N_m\|(I{-}P^*)(\bar{s}_m{-}\bar{s})\|_w^2.
\]
Summing this over $m$ and using $\sum_m\gamma_{im}=1$ for each $i$:
\begin{align*}
  \sum_m\sum_i\gamma_{im}\|(I{-}P^*)(s^{(i)}{-}\bar{s}_m)\|_w^2
  &= \sum_i\|(I{-}P^*)(s^{(i)}{-}\bar{s})\|_w^2
    - \underbrace{\sum_m N_m\|(I{-}P^*)(\bar{s}_m{-}\bar{s})\|_w^2}_{\geq\,0}\\
  &\leq \sum_i\|(I{-}P^*)(s^{(i)}{-}\bar{s})\|_w^2
    = \varepsilon_{\mathrm{global}}^K. \qquad\square
\end{align*}
\end{proof}
