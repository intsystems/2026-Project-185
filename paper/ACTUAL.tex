...
\paragraph{TEMPO regime structure (2D Darcy flow, $M{=}5$).}
Figure~\ref{fig:tempo_phase1_darcy} shows the TEMPO Phase~1 regime structure
on 2D Darcy flow with $M{=}5$ regimes discovered by EM.
As in the 1D Burgers' case, regimes span multiple $\beta$ values,
confirming geometry-driven rather than parameter-driven partition.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{TEMPO/main/figs/TEMPO_phase1_darcy_M5.png}
\caption{TEMPO regime structure on 2D Darcy flow ($M{=}5$): UMAP coloured
  by regime assignment (\textit{left}) and by $\beta$ (\textit{right}).}
