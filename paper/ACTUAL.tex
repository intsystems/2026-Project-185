...
When the governing parameter drives qualitative change in solution structure, a single basis must fit incompatible geometries and prediction quality suffers.
...
TEMPO(POD) outperforms the joint POD-DeepONet baseline by 13--50\% on Burgers' and up to $4\times$ on Darcy flow, where per-parameter specialists fail catastrophically out-of-distribution.
...
$\kappa$ predicted from $u_0$ & \textemdash & $+0.024$ ($+7\%$) & \multicolumn{2}{c}{not predictable ($\beta \perp a(x)$)} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Global POD basis on 1D Burgers' ($\nu{=}1.0$): singular value spectrum (\textit{left}) and cumulative explained variance (\textit{right}).}
...
Figure~\ref{fig:basis_rec} shows representative Phase~1 reconstructions from the Fourier NeuralPOD basis.
...
\caption{TEMPO(NeuralPOD) on 1D Burgers' ($M{=}3$): predictions exhibit high-frequency noise (L2 $0.46$--$0.54$); layout as in Figure~\ref{fig:tempo_pod_burgers_rec}.}
...
\paragraph{TEMPO regime discovery and training dynamics (2D Navier-Stokes).}
Figures~\ref{fig:ns_em_convergence} and~\ref{fig:ns_training_dynamics} show the EM convergence and online training dynamics for TEMPO(POD) on 2D Navier-Stokes ($M{=}4$).
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figs/ns_training_dynamics.png}
\caption{Online training dynamics for TEMPO(POD) on 2D Navier-Stokes ($M{=}4$).}
\label{fig:ns_training_dynamics}
\end{figure}

\paragraph{TEMPO reconstructions (2D Navier-Stokes).}
Figures~\ref{fig:ns_reconstruction} and~\ref{fig:ns_reconstruction_3d} show representative predictions from TEMPO(POD) on 2D Navier-Stokes.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figs/ns_reconstruction_sample0.png}
\caption{TEMPO(POD) on 2D Navier-Stokes ($M{=}4$): ground truth (\textit{left}), prediction (\textit{center}), point-wise absolute error (\textit{right}).}
\label{fig:ns_reconstruction}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figs/ns_reconstruction_3d_sample0.png}
\caption{TEMPO(POD) on 2D Navier-Stokes ($M{=}4$): 3D vorticity field visualization.}
