...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figs/TEMPO_graphical_highlights.png}
  \caption{\textbf{TEMPO pipeline.}
    Phase~1 (offline): snapshots are soft-assigned to $M$ regimes via EM,
    and a dedicated basis $\{\Phi_m\}$ is fitted per regime.
    Phase~2 (online): a GatingNet routes each input to regime weights;
    $M$ BranchNets predict expansion coefficients; the output is a weighted
    sum of local reconstructions.}
  \label{fig:pipeline}
\end{figure}
...
The number of regimes $M \in \{2,3,4\}$ is selected by a two-criterion rule:
BIC on the TEMPO EM log-likelihood (lower is better, penalises model complexity)
and mean assignment entropy $H$ (higher gain signals a genuinely new regime);
...
outperforming the global joint baseline by 13--50\% across all~$\nu$.
TEMPO(NeuralPOD) recovers the same regimes but does not surpass the joint
baseline, suggesting nonlinear basis fitting remains harder in the
multi-regime setting.
...
TEMPO(POD) reduces error by $3$--$5\times$ over the joint baseline
(e.g., $\beta{=}0.1$: $0.775$ vs.\ $2.557$; $\beta{=}100$: $0.073$
vs.\ $0.347$).
TEMPO(NeuralPOD) recovers dramatically over its non-regime baseline
($\beta{=}0.1$: $3.813$ vs.\ $72.9$) but does not match TEMPO(POD).
Recent methods (FNO, CNO) employ fundamentally different operator learning paradigms
without explicit basis decomposition; TEMPO's contribution is discovering and
leveraging latent regime structure through interpretable, locally-adapted bases.
...
\multirow{3}{*}{DeepONet~\cite{Lu2019}}
  & 0.001 & \underline{0.536} & 0.474 & 0.382 \\
...
\multirow{4}{*}{FNO~\cite{Li2021}}
  & 0.001 & \underline{0.426} & 0.529 & 0.623 \\
...
DeepONet~\cite{Lu2019}
  & all & 0.533 & 0.472 & 0.289 \\
POD-DeepONet~\cite{Lu2022}
  & all & 0.460 & 0.339 & 0.210 \\
FNO~\cite{Li2021}
  & all & 0.427 & 0.298 & 0.202 \\
CNO~\cite{Raonic2023} \small{(2.0M)}
  & all & 0.361 & \textbf{0.162} & \textbf{0.133} \\
...
\multirow{5}{*}{DeepONet~\cite{Lu2019}}
  & 0.01  & \underline{4.197} & 0.615 & 0.944 & 0.994 & 0.999 \\
...
\multirow{5}{*}{FNO~\cite{Li2021}}
  & 0.01  & \underline{0.529} & 0.791 & 0.967 & 0.997 & 0.9998 \\
...
\multirow{4}{*}{CNO~\cite{Raonic2023} \small{(2.0M)}}
  & 0.01  & \underline{0.388} & 0.929 & 1.000 & 1.001 & 1.000 \\
...
DeepONet~\cite{Lu2019}
  & all
  & \textemdash & 76.53 & 8.503 & 1.352 & 0.493 \\
POD-DeepONet~\cite{Lu2022}
  & all
...
FNO~\cite{Li2021}
  & all
  & \textemdash & 0.380 & 0.083 & 0.023 & 0.008 \\
CNO~\cite{Raonic2023} \small{(2.0M)}
  & all
  & \textemdash & \textbf{0.133} & \textbf{0.026} & \textbf{0.008} & \textbf{0.005} \\
TEMPO(POD)~\textit{(ours)}
  & $M{=}5$
  & \textemdash & 0.775 & 0.178 & 0.136 & 0.073 \\
TEMPO(NeuralPOD)~\textit{(ours)}
  & $M{=}5$
...
\multirow{4}{*}{DeepONet~\cite{Lu2019}}
  & 100   & \underline{1.000} & 1.002 & 1.003 & 1.004 & \multirow{4}{*}{---} \\
  & 1000  & 1.000 & \underline{1.000} & 1.000 & 1.000 & \\
  & 3600  & 1.002 & 1.000 & \underline{1.000} & 1.000 & \\
  & 10000 & 1.001 & 1.000 & 1.000 & \underline{1.000} & \\[4pt]
\multirow{4}{*}{POD-DeepONet~\cite{Lu2022}}
  & 100   & \underline{0.082} & 0.748 & 0.865 & 0.877 & \multirow{4}{*}{---} \\
  & 1000  & 0.667 & \underline{0.129} & 0.671 & 0.812 & \\
  & 3600  & 0.660 & 0.392 & \underline{0.180} & 0.623 & \\
  & 10000 & 0.934 & 0.687 & 0.556 & \underline{0.345} & \\[4pt]
NeuralPOD-DeepONet~\textit{(ours)}
  & 1000  & 0.762 & \underline{0.198} & 0.607 & 0.761 & --- \\[4pt]
\multirow{4}{*}{FNO~\cite{Li2021}}
  & 100   & \underline{0.013} & 0.262 & 0.368 & 0.437 & \multirow{4}{*}{1.50} \\
  & 1000  & 0.287 & \underline{0.033} & 0.138 & 0.227 & \\
  & 3600  & 0.381 & 0.150 & \underline{0.039} & 0.128 & \\
  & 10000 & 0.461 & 0.260 & 0.126 & \underline{0.042} & \\
\midrule
\multicolumn{7}{l}{\textit{Joint training (all Re)}} \\[2pt]
POD-DeepONet~\cite{Lu2022}
  & all & 0.092 & 0.132 & 0.141 & 0.145 & --- \\
FNO~\cite{Li2021}
  & all & \textbf{0.009} & \textbf{0.020} & \textbf{0.024} & \textbf{0.027} & 0.61 \\
NeuralPOD-DeepONet~\textit{(ours)}
  & all & \multicolumn{4}{c}{\textit{pend.}} & \textit{pend.} \\
CNO~\cite{Raonic2023}
  & all & \multicolumn{4}{c}{\textit{pend.}} & \textit{pend.} \\
TEMPO(POD)~\textit{(ours)}
  & $M{=}4$ & 0.105 & 0.153 & 0.167 & 0.181 & \textbf{0.05} \\
TEMPO(NeuralPOD)~\textit{(ours)}
  & \textit{pend.} & \multicolumn{4}{c}{\textit{pend.}} & \textit{pend.} \\
\bottomrule
\end{tabular}
\end{table}
...
BIC increases monotonically with $M$ (favouring $M{=}2$ by complexity alone),
...
$\boldsymbol{\pi}{=}(0.324,\,0.267,\,0.409)$: knowing~$\nu$ alone does not
predict regime membership.
...
behaviour before recording begins.
...
\caption{Regime count selection on 1D Burgers'.
  BIC $= {-2\log\mathcal{L} + k\log N}$ (lower is better; penalises complexity);
  $H$: mean assignment entropy (higher gain signals a new genuine regime);
  $\bar{\varepsilon}$: mean relative $L^2$ test error.
  BIC increases monotonically, favouring $M{=}2$ by complexity alone.
