...
a single jointly-trained basis on benchmarks with structured regimes.
...
NeuralPOD-DeepONet~\textit{(ours)}
  & all & \multicolumn{4}{c}{\textit{pend.}} & 4.87 \\
CNO~\cite{Raonic2023}
...
TEMPO(POD) outperforms the joint POD-DeepONet baseline by 13--50\% on Burgers' and up to $4\times$ on Darcy flow; on Navier-Stokes, TEMPO matches the POD baseline.
Discovered regimes span multiple parameter values, confirming that the partition reflects solution geometry and physical structure jointly.
...
Extending regime discovery to continuous parameter spaces and three-dimensional problems remains an open direction.
...
\paragraph{TEMPO regime structure (2D Darcy flow).}
Figures~\ref{fig:tempo_phase1_darcy}--\ref{fig:tempo_phase1_darcy_npod} show
the TEMPO Phase~1 regime structure on 2D Darcy flow for POD and
NeuralPOD bases ($M{=}4$ shown for interpretability). Regimes span multiple $\beta$ values.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figs/TEMPO_phase1_darcy_pod_M4.png}
\caption{TEMPO(POD) on 2D Darcy flow ($M{=}4$, shown for interpretability; quantitative results in Table~\ref{tab:darcy} use $M{=}5$). Layout as in Figure~\ref{fig:umap_em}.}
...
\caption{TEMPO(NeuralPOD) on 2D Darcy flow ($M{=}4$, shown for interpretability; quantitative results in Table~\ref{tab:darcy} use $M{=}5$). Layout as in Figure~\ref{fig:umap_em}.}
