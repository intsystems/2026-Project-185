...
Figure~\ref{fig:umap_em} confirms regime structure in both the UMAP
(\textit{left/centre}) and the POD coefficient space (\textit{right}):
each manifold branch spans multiple~$\nu$, confirming the partition
reflects solution geometry rather than the physical parameter directly.
...
\paragraph{TEMPO reconstructions.}
Figures~\ref{fig:tempo_pod_burgers_rec}--\ref{fig:tempo_npod_darcy_rec}
show representative predictions from all four TEMPO variants.
...
on 2D Darcy flow with $M{=}5$ regimes.
