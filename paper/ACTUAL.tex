...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item On 1D Burgers' and 2D Darcy flow, per-parameter specialists achieve
    low in-distribution error but fail catastrophically outside their training
    value; TEMPO(POD) trained jointly achieves lower average error across the
    full parameter range than any single-parameter specialist and outperforms
    a jointly-trained model without regime discovery, with discovered regimes
    reflecting both solution geometry and physical parameter structure.
\end{itemize}
...
see Table~\ref{tab:regime_selection} (Appendix~\ref{app:exp}).
...
POD-DeepONet and NeuralPOD-DeepONet trained jointly on all parameter values
without regime discovery serve as baselines for TEMPO.
...
Table~\ref{tab:results} reports the full cross-viscosity error matrix.
Each per-$\nu$ specialist achieves low in-distribution error (diagonal), but
fails sharply outside its training regime: a model trained on $\nu{=}0.1$
reaches $0.048$ in-distribution yet $0.461$ on $\nu{=}0.001$, a ten-fold
increase. These specialists are not usable in practice across the full
parameter range.
TEMPO(POD) trained jointly on all viscosities reduces error by 31--50\%
relative to a single joint POD-DeepONet baseline across all~$\nu$.
TEMPO(NeuralPOD) shows consistent regime structure but higher absolute error,
reflecting the added difficulty of fitting nonlinear bases in the joint setting.

\paragraph{2D Darcy flow.}
Table~\ref{tab:darcy} shows the cross-$\beta$ error matrix.
Per-$\beta$ specialists fail even more severely: the Darcy solution scales
linearly with~$\beta$, so a model trained on one value predicts completely
wrong amplitudes on others, with errors growing by one order of magnitude
per decade in~$\beta$ and reaching ${>}1000$ for the most distant pair.
TEMPO(POD) trained jointly on $\beta\in\{0.1,1,10,100\}$ outperforms the
joint POD-DeepONet baseline by up to $3\times$ on the hardest regime
($\beta{=}0.1$), recovering accurate predictions across all values.
...
\multirow{4}{*}{NeuralPOD-DeepONet~\textit{(ours)}}
  & 0.001 & \underline{0.392} & 0.288 & 0.535 \\
  & 0.01  & 0.391 & 0.284 & 0.541 \\
  & 0.1   & 0.426 & \underline{0.229} & 0.371 \\
  & 1.0   & 0.483 & 0.315 & \underline{0.183} \\
\midrule
\multicolumn{5}{l}{\textit{Joint training ($\nu\in\{0.001,0.1,1.0\}$)}} \\[2pt]
POD-DeepONet~\cite{Lu2022}
  & all & 0.460 & 0.339 & 0.210 \\
TEMPO(POD)~\textit{(ours)}
  & $M{=}3$ & \textbf{0.317} & \textbf{0.168} & \textbf{0.182} \\
TEMPO(NeuralPOD)~\textit{(ours)}
  & $M{=}3$ & 0.571 & 0.502 & 0.376 \\
\bottomrule
\end{tabular}
\end{table}
...
We select $M{=}3$ based on BIC and an entropy elbow: the mean assignment
entropy $H$ gains $+0.30$ from $M{=}2$ to $M{=}3$ but only $+0.18$ from
$M{=}3$ to $M{=}4$, indicating the third regime captures genuine structure
that a fourth does not (Table~\ref{tab:regime_selection},
...
The per-viscosity EM responsibilities closely match the global mixture
weights $\boldsymbol{\pi}{=}(0.324,\,0.267,\,0.409)$ across all $\nu$
(Table~\ref{tab:gating}, Appendix~\ref{app:exp}), confirming that regime
membership is driven by the geometry of~$u_0$, not by~$\nu$.

\paragraph{Regime structure.}
Figure~\ref{fig:umap_em} shows that each manifold branch spans
multiple~$\nu$ values, confirming the partition reflects solution
...
\caption{TEMPO(POD) regime structure on 1D Burgers' ($M{=}3$, 20 EM iterations).
  \textit{Left:} UMAP coloured by regime.
...
Per-parameter specialists achieve low in-distribution error but are unusable across the full parameter range — errors grow by orders of magnitude out-of-distribution.
TEMPO(POD) trained jointly outperforms a single joint baseline by 31--50\% on Burgers' and up to $3\times$ on Darcy flow, while discovered regimes span multiple parameter values, confirming that the partition reflects solution geometry and physical structure jointly.
TEMPO(NeuralPOD) demonstrates that the regime discovery framework extends naturally to nonlinear bases; closing the gap with TEMPO(POD) in the joint training setting remains an open direction.
...
global mixture weights~$\pi_m$; near-uniform agreement across $\nu$ confirms
geometry-driven rather than parameter-driven regime membership.
...
\caption{Mean EM assignment weights per viscosity (1D Burgers',
  ...
  Each row sums to~$1$; near-uniform agreement across $\nu$ confirms
  regime membership is driven by~$u_0$ geometry, not by~$\nu$.}
\label{tab:gating}
\begin{tabular}{c c c c}
\toprule
...
\caption{Global POD basis on 1D Burgers' ($\nu{=}1.0$): singular value spectrum
  (\textit{left}) and cumulative explained variance (\textit{right}); $K{=}30$ modes attain $99.99\%$.}
...
Figure~\ref{fig:basis_spectrum} shows the singular-value decay of the global POD
basis alongside the weighted residual decay of the Fourier NeuralPOD basis.
Figure~\ref{fig:basis_modes} shows the first $K$ spatiotemporal mode functions
for both bases; POD modes are globally supported, while NeuralPOD modes
adapt their spatial localisation to the data.
Figure~\ref{fig:basis_modes_nu001} shows the same comparison for $\nu{=}0.01$:
NeuralPOD modes concentrate near the shock front while POD modes remain globally supported.
Figure~\ref{fig:basis_reconstruction} compares per-sample reconstructions from
both bases: ground truth, POD prediction, POD error, Fourier NeuralPOD prediction,
and Fourier NeuralPOD error.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{TEMPO/main/figs/basis_spectrum_nu1.0.png}
\caption{POD singular-value decay (\textit{left}) and Fourier NeuralPOD
  residual decay (\textit{right}) on 1D Burgers' ($\nu{=}1.0$).}
\label{fig:basis_spectrum}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{TEMPO/main/figs/basis_modes_nu1.0.png}
\caption{First $K$ basis modes on 1D Burgers' ($\nu{=}1.0$):
  POD (\textit{top}) vs.\ Fourier NeuralPOD (\textit{bottom}).}
\label{fig:basis_modes}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{TEMPO/main/figs/basis_modes_nu0.01.png}
\caption{First $K$ basis modes on 1D Burgers' ($\nu{=}0.01$):
  POD (\textit{top}) vs.\ Fourier NeuralPOD (\textit{bottom}).}
...
\caption{POD-DeepONet test-set error distribution on 1D Burgers' ($\nu{=}1.0$):
  error histogram (\textit{left}) and sorted errors (\textit{right}).}
...
\paragraph{TEMPO reconstructions (1D Burgers').}
Figures~\ref{fig:tempo_pod_burgers_rec}--\ref{fig:tempo_npod_burgers_rec}
show representative space--time predictions from the two TEMPO variants on 1D Burgers'.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{TEMPO/main/figs/tempo_pod_burgers_reconstruct.png}
\caption{TEMPO(POD) on 1D Burgers' ($M{=}3$): ground truth (\textit{left}),
  prediction (\textit{centre}), point-wise absolute error (\textit{right}).}
...
\caption{TEMPO(NeuralPOD) on 1D Burgers' ($M{=}3$). Layout as in
  Figure~\ref{fig:tempo_pod_burgers_rec}.}
\label{fig:tempo_npod_burgers_rec}
\end{figure}

\paragraph{TEMPO regime structure (1D Burgers', NeuralPOD basis).}
Figure~\ref{fig:umap_em_npod} shows the TEMPO(NeuralPOD) regime structure on
1D Burgers' ($M{=}3$). The partition mirrors TEMPO(POD) but with sharper
assignments, reflecting the higher expressivity of the NeuralPOD basis.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figs/TEMPO_phase1_burgers_npod_M3.png}
\caption{TEMPO(NeuralPOD) on 1D Burgers' ($M{=}3$, $H{=}0.23$, cf.\ $H{=}0.84$ for POD). Layout as in Figure~\ref{fig:umap_em}.}
\label{fig:umap_em_npod}
\end{figure}

\paragraph{TEMPO regime structure (2D Darcy flow, $M{=}4$).}
Figures~\ref{fig:tempo_phase1_darcy}--\ref{fig:tempo_phase1_darcy_npod} show
the TEMPO Phase~1 regime structure on 2D Darcy flow ($M{=}4$) for POD and
NeuralPOD bases. Regimes span multiple $\beta$ values, confirming
geometry-driven rather than parameter-driven partition.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figs/TEMPO_phase1_darcy_pod_M4.png}
\caption{TEMPO(POD) on 2D Darcy flow ($M{=}4$, $H{=}0.75$, $\boldsymbol{\pi}{=}(0.22,\,0.28,\,0.28,\,0.22)$). Layout as in Figure~\ref{fig:umap_em}.}
\label{fig:tempo_phase1_darcy}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figs/TEMPO_phase1_darcy_npod_M4.png}
\caption{TEMPO(NeuralPOD) on 2D Darcy flow ($M{=}4$, $H{=}0.68$). Layout as in Figure~\ref{fig:umap_em}.}
\label{fig:tempo_phase1_darcy_npod}
\end{figure}

\paragraph{TEMPO reconstructions (2D Darcy flow).}
Figures~\ref{fig:tempo_pod_darcy_rec}--\ref{fig:tempo_npod_darcy_rec}
show representative predictions from the two TEMPO variants on 2D Darcy flow.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{TEMPO/main/figs/tempo_pod_darcy_reconstruct.png}
\caption{TEMPO(POD) on 2D Darcy flow ($M{=}5$): ground truth (\textit{left}),
  prediction (\textit{centre}), point-wise absolute error (\textit{right}).}
...
\caption{TEMPO(NeuralPOD) on 2D Darcy flow ($M{=}5$). Layout as in
  Figure~\ref{fig:tempo_pod_darcy_rec}.}
