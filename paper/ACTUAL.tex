...
\paragraph{TEMPO regime discovery \& reconstructions (2D Navier-Stokes).}
Figure~\ref{fig:ns_rec_collage} shows the EM convergence and representative predictions from TEMPO(POD) on 2D Navier-Stokes ($M{=}4$).
...
\begin{subfigure}{\textwidth}
  ...
  \caption{Ground truth (\textit{left}), prediction (\textit{center}), point-wise absolute error (\textit{right}).}
...
\begin{subfigure}{\textwidth}
  ...
  \caption{3D vorticity field visualization.}
  \label{fig:ns_reconstruction_3d}
\end{subfigure}
\caption{TEMPO(POD) on 2D Navier-Stokes ($M{=}4$): EM convergence (\textit{top}); representative predictions (\textit{middle}, \textit{bottom}).}
