...
Usage:
  Specialist: python code/scripts/train_cno_navier_stokes.py --re_values 1000
  Joint:      python code/scripts/train_cno_navier_stokes.py --re_values 100 1000 3600 10000
"""
import argparse
import json
import os
...
_CNO_SEARCH = [
    ...
    str(_PROJECT_ROOT.parent / "CNO" / "CNO2d_simplified"),
    str(_PROJECT_ROOT / "CNO2d_simplified"),
...
def predict_batched(model, X_tensor, device, batch_size=16):
...
def parse_args():
    ...
    p.add_argument("--n_res_neck",      type=int,   default=2,
...
def main():
    ...
    if args.run_name:
        ...
        RUN_NAME = "cno_joint_navier_stokes_v1"
    else:
        RUN_NAME = f"cno_navier_stokes_re{args.re_values[0]}_v1"
    ...
    print(f"Building CNO inputs {Nx}x{Ny} ...")
    X_train = torch.tensor(build_cno_input(u0_np[train_idx], kappa_train, Nx, Ny))
    X_test  = torch.tensor(build_cno_input(u0_np[test_idx],  kappa_test,  Nx, Ny))
    ...
    print(f"CNO params: {n_params:,}  out_channels={out_ch}")
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        if epoch % args.log_every == 0 or epoch == 1:
            ...
            print(f"  epoch {epoch:4d} | train_l1={avg:.4e} | val_rel_l2={err_val:.4f} | {time.time()-t0:.0f}s")
        else:
            print(f"  epoch {epoch:4d} | train_l1={avg:.4e} | {time.time()-t0:.0f}s")
    ...
    ax1.set_title("CNO NS training dynamics", fontweight="bold")
    plt.tight_layout()
    ...
    if not is_joint:
        ...
        for re_eval in _ALL_RE:
            ...
            X_re = torch.tensor(build_cno_input(u0_re_test, kap_re, Nx, Ny))
    ...
    if is_joint and len(re_keys) > 1:
        ...
        ax.set_title("CNO - per-Re test errors (joint, NS)", fontweight="bold")
    ...
    ax.spines[["top", "right"]].set_visible(False)
    plt.suptitle("CNO Navier-Stokes - test errors", fontweight="bold")
    plt.tight_layout()
    ...
    for si, idx in enumerate(idxs):
        ...
        fig.suptitle(f"CNO NS  Re={re_i}  rel-L2={rl2:.4f}", fontweight="bold")
